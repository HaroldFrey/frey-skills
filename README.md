...
| 分类 | Skill |
|------|-------|
| 🔧 执行型 | `pwd` `ls` `skills` `git-push` `git-pull` |
| 📋 预处理 | `req-analysis` |
...
### `/git-pull`
**检查远程更新并同步本地**——两种模式自动判断：传入目录本身是 git 仓库 → 单仓库模式；不是仓库 → **递归扫描其所有子目录**，自动发现全部 git 仓库并逐个检查同步。

| 参数 | 说明 |
|------|------|
| 无参数 | 检查当前目录（按模式判断） |
| `/git-pull <目录1> [<目录2> ...]` | 单仓库模式（目录是仓库）或递归扫描模式（目录是根目录） |

| 仓库状态 | 处理 |
|---------|------|
| 落后远程且工作区干净 | 单仓库：✅ 自动 pull；**扫描模式：🛑 报告后等用户决定** |
| 落后但有未提交修改 | ⚠️ 跳过并告知（不擅自覆盖） |
| 本地超前 / 分叉 / 无远程 | ⚠️ 汇总告知 |
| 网络失败 | ❌ 重试 3 次（10 秒间隔） |

> 💡 例：`/git-pull D:\Stduy` 一次扫描发现并同步该目录下所有 git 仓库。

### `/git-push`
**智能推送到 GitHub**——两种模式自动判断：

| 模式 | 触发 | 行为 |
|------|------|------|
| 直接推送 | 无参数 / 目录是 git 仓库 | 自动 add → commit → push |
| 扫描模式 | 目录不是 git 仓库 | 递归扫描所有子仓库，列出需要推送的（🔄未推送提交 / 📝未提交修改），🛑 由用户决定推哪些 |

> ⚠️ 首次推送提醒手动创建 GitHub 仓库；网络失败自动重试 3 次（10 秒间隔）；扫描模式不擅自推送，等用户确认。
